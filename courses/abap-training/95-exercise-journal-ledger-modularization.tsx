...
/**
 * 特別演習④ Part A 完成形：「プログラムの構造化」に従い、START_PROGRAM を
 * FORM/PERFORM へリファクタリングした構造化済みコード。
 */
const STRUCTURED_PROGRAM = `REPORT create_report_3
  NO STANDARD PAGE HEADING
  LINE-SIZE 200
  LINE-COUNT 58.
          ...
          content: (
            <>
              ...
              <Callout variant="note">
                ...
                Part B 以降（GUIステータスの追加・ダウンロード処理など）は、この構造化が土台になります。
              </Callout>
              <h3>このパートでやること</h3>
              <ul>
                <li>
...
" 詳細は FORM 定義側へ`}</pre>
              ...
              <InfoPanel title="構造化で得られる3つの良いこと（第10章）" variant="reference">
                <ul>
                  ...
                  <li>
                    <strong>育てやすい</strong> … Part B（GUIステータス・ダウンロード）を足すとき、<code>FORM</code> を追加しやすい
                  </li>
                </ul>
              </InfoPanel>
              <Callout variant="note">
...
ENDFORM.`}
        ...
        },
        {
          ...
          plainText:
            "A-⑧ Part A 完成コード全文 create_report_3（構造化版）。イベントは PERFORM のみ、処理は FORM へ分割、lv_* は f_proc_write のローカル変数、SORT は f_proc_sort。出力結果は演習③と同じ。Reveal で全文を開いて出発点コードと見比べる。次は Part B（GUIステータス・ダウンロード）。",
          content: (
            <>
          ...
          content: (
            <>
              ...
              <Dialog speaker="closing">
                ...
                <br />
                この土台の上に、Part B で GUIステータスやダウンロード処理を足していきましょう。
              </Dialog>
            </>
          ),
        },
