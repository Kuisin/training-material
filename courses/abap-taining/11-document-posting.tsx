...
export default function DocumentPostingLesson() {
  ...
  P->>P: 履歴・結果を記録/表示`}
        ...
        },
        {
          title: "BAPIはなぜ必要か",
          plainText:
            "BAPI ＝ SAP公式の「安全な登録窓口」\n伝票テーブルに直接書き込むのは危険（整合性が崩れる）。BAPI はSAPが用意した正しい手順で登録してくれる公式の窓口。通せば必要なチェックや関連更新もまとめて面倒を見てくれる。\nつまずき：テーブルに直接 INSERT すればいいは厳禁。会計伝票は必ず BAPI など正式な手段で登録する。\nAくん：APIを通すことで内部の整合性ルールを破らずに済むんですね。\n先生：そう。窓口を通すから、関連する複数テーブルも矛盾なく更新されます。",
          content: (
            <>
        ...
        },
        {
          title: "ロックは何を守るか",
          plainText:
            "ロック ＝ 「使用中」の札\n2人が同時に同じデータを書き込むと記録が壊れる。ロックオブジェクトはトイレの使用中札のように今このデータは私が触っていますと示し二重更新を防ぐ。\nBちゃん：試着室の使用中札と同じですね。空くまで待つ。\n先生：登録のような書き込みではロックで守ってから処理するのが基本。終わったら必ず解放します。",
          content: (
            <>
              <h2>ロック ＝ 「使用中」の札</h2>
              ...
              <Dialog speaker="teacher">
                登録のような“書き込み”では、ロックで守ってから処理するのが基本です。そして終わったら必ず解放します。札を出したまま帰ると、ほかの人がずっと使えなくなります。
              </Dialog>
            </>
          ),
        },
        {
          title: "登録を支える部品",
          plainText:
            "登録を支える部品（位置づけ）\nアドオンテーブル：標準にない情報を保存する自社用の追加の表（取込履歴など）\n汎用モジュール：部品化された処理のまとまり（FORMの強力版・他からも呼べる）\nBAPI：SAP公式の業務処理窓口（伝票登録など）\nロックオブジェクト：同時更新を防ぐ使用中札\nこの章のABAPキーワード：BAPI / ロックオブジェクト / 汎用モジュール / アドオンテーブル / コミット。",
          content: (
            <>
              <h2>登録を支える部品（位置づけ）</h2>
              <ul>
                <li><strong>アドオンテーブル</strong>：標準にない情報を保存する“自社用の追加の表”（取込履歴など）</li>
                <li><strong>汎用モジュール</strong>：部品化された処理のまとまり（FORMの強力版・他からも呼べる）</li>
                <li><strong>BAPI</strong>：SAP公式の業務処理窓口（伝票登録など）</li>
                <li><strong>ロックオブジェクト</strong>：同時更新を防ぐ「使用中」札</li>
              </ul>
              <Callout variant="tip">
                この章のABAPキーワード：BAPI / ロックオブジェクト / 汎用モジュール / アドオンテーブル / コミット。
              </Callout>
        ...
        },
        {
          title: "対話で整理",
          plainText:
            "対話で整理\n先生：この章は見る処理から書き込む処理へ責任が変わる転換点。登録では検証・ロック・BAPI・コミット・結果記録の順を守り、整合性を壊さないことが最優先。\nAくん：BAPIを公式窓口として使うことで内部整合チェックや関連更新をSAP標準ルールに沿って実行できる。テーブル直接更新を避ける理由が論理的に理解できました。\nBちゃん：ロックは使用中の札で同時更新を防ぐ安全装置。登録が終わるまでの手順を飛ばさず、結果を残すところまでを1セットで扱います。",
          content: (
            <>
              <h2>対話で整理</h2>
              <Dialog speaker="teacher">
