...
- **原画（保持用）**: `courses/sap-products-services/image/originals/<ファイル名>.png` — リポジトリに残す。サイトには出さない。
- **配信用**: `courses/sap-products-services/image/<ファイル名>.webp` — `pnpm run optimize:images` で原画から生成。
